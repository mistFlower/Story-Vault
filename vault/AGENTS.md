...
reference/novelpia.md, reference/joara.md
- 대상 플랫폼의 연재 기준 리서치 원문.
- 앱이 플랫폼 선택 시 자동으로 넣는다. 직접 수정하지 않는다.
- 대상이 아닌 플랫폼 파일은 앱이 지운다. 있는 파일만 읽는다.
...
두 기준을 섞지 않는다. 근거는 `reference/` 의 플랫폼 문서에 있다.

두 곳에 동시 연재하는 작품이면 한 회차가 양쪽 기준을 모두 만족해야 한다.
조아라 기준이 더 길므로 조아라를 먼저 맞춘다.

대상 플랫폼은 작품 시작 시 한 번 정하고 바꾸지 않는다.
연재 도중 바꾸면 이미 쓴 회차가 기준에서 어긋난다.
